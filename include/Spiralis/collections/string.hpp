...
private:
    ...
    struct __big_mode{
        ...
        __big_mode(){}
        __big_mode(__big_mode&& other) : _data(other._data), _size(other._size), _capacity(other._capacity){
            other._data = nullptr;
            other._size = 0;
            other._capacity = 0;
        }
        ...
        void reserve(size_type min_bytes){
        ...
        void reserve_exact(size_type bytes){
    ...
    SP_FORCEINLINE __big_mode& B() noexcept { return _str._B; }
    SP_FORCEINLINE const __big_mode& B() const noexcept { return _str._B; }

    SP_FORCEINLINE __small_mode& S() noexcept { return _str._S; }
    SP_FORCEINLINE const __small_mode& S() const noexcept { return _str._S; }
    ...
    SP_FORCEINLINE void set_small_length(size_type len) {
        S()._flag = (S()._flag & 0x80) | (len & 0x1F);
    }

    SP_FORCEINLINE void _inflate(size_type new_cap) {
        char temp[23];
...
SP_FORCEINLINE string_impl& __priv_assign(const char* other, size_type len){
    if (len <= 22) {
...
SP_FORCEINLINE string_impl& __priv_push_back(const char element){
...
SP_FORCEINLINE string_impl& __priv_append(const char* other, size_type len){
...
SP_FORCEINLINE string_impl& __priv_insert(size_type index, const char* other, size_type len){
    size_type current_len = length();
...
SP_FORCEINLINE string_impl& __priv_replace(size_type index, size_type len_to_replace, const char* other, size_type len){
    size_type current_len = length();
...
SP_FORCEINLINE iterator begin() noexcept { return iterator(data()); }
SP_FORCEINLINE const_iterator begin() const noexcept { return const_iterator(data()); }
SP_FORCEINLINE const_iterator cbegin() const noexcept { return const_iterator(data()); }

SP_FORCEINLINE iterator end() noexcept { return iterator(data() + size()); }
SP_FORCEINLINE const_iterator end() const noexcept { return const_iterator(data()+size()); }
SP_FORCEINLINE const_iterator cend() const noexcept { return const_iterator(data()+size()); }

SP_NODISCARD SP_FORCEINLINE bool is_empty() const{ return size() == 0;}
_SP_FUNC_NIP_ bool is_small() const{ return S()._flag & 0x80; }
_SP_FUNC_NIP_ bool is_big() const{ return !is_small(); }
size_type size() const{ return is_small() ? (S()._flag & 0x1F) : B()._size; }
size_type length() const{ return is_small() ? (S()._flag & 0x1F) : B()._size; } // alias for size
size_type capacity() const{ return is_small() ? 22 : B()._capacity; }

char* data() { return is_small() ? S()._data : B()._data; }
const char* data() const { return is_small() ? S()._data : B()._data; }
const char* c_str() const { return (is_small() ? S()._data : B()._data); }

SP_NODISCARD SP_FORCEINLINE string_impl C() const { return string_impl(*this); }
SP_NODISCARD SP_FORCEINLINE string_impl clone() const { return string_impl(*this); }
...
_SP_FUNC_NIFP_ bool operator!=(const string_impl& other)const{
    return !(*this==other);
}

SP_FLATTEN string_impl& operator=(const string_impl& other){
...
string_impl& operator=(const char* other){
    return __priv_assign(other, strlen(other));
}

SP_HOT SP_FORCEINLINE string_impl& operator+=(char c){ return __priv_push_back(c); }
SP_HOT SP_FORCEINLINE string_impl& operator+=(const char* other){ return __priv_append(other, strlen(other)); }
SP_HOT SP_FORCEINLINE string_impl& operator+=(const string_impl<_safety_level, Allocator>& other){ return __priv_append(other.c_str(), other.size()); }
SP_HOT SP_FORCEINLINE string_impl& operator+=(const string_impl<_other_safety_level, Allocator>& other){ return __priv_append(other.c_str(), other.size()); }

SP_HOT SP_FORCEINLINE string_impl operator*(size_type multiplier) const {
    string_impl result;
...
SP_HOT SP_FORCEINLINE string_impl& operator*=(size_type multiplier){
    size_type total_len = size() * multiplier;
    if(total_len <= 22){
        string_impl copy = C();
        clear();
...
SP_FORCEINLINE char& operator[](size_type index){
    return is_small() ? S()._data[index] : B()._data[index];
}

SP_FORCEINLINE const char& operator[](size_type index) const{
    return is_small() ? S()._data[index] : B()._data[index];
}
...
SP_FORCEINLINE string_impl(const string_impl& other) : string_impl(){
    __priv_assign(other.c_str(), other.length());
}

SP_FORCEINLINE string_impl(string_impl&& other) : string_impl(){
    *this = sp::move(other);
}



string_impl(size_type count, char ch) : string_impl() {
    if(count <= 22){
...
// start and end idx
string_impl(const char* str, size_type len) : string_impl() {
    if (len <= 22) {
        memcpy(S()._data, str, len);
        S()._data[len] = '\0';
        set_flag<true>();
...
SP_FORCEINLINE void clear_keep_capacity() noexcept{
    if(is_big()){
        B()._size = 0;
        B()._data[0] = '\0';
    }else{
...
SP_COLD void shrink_to_fit(){
    SP_IF_EXPECT(is_big()) B().shrink_to_fit();
}
SP_HOT void reserve(size_type new_cap){
    size_type len = size();
...
SP_HOT void reserve_exact(size_type new_cap){
    size_type len = size();
    SP_IF_EXPECT(new_cap>size()&&new_cap>22){
        if(is_small()) _inflate(new_cap+1);
...
SP_HOT string_impl& trim_start(){
    size_type i = 0;
    while(i<size() && isspace(c_str()[i])) i++;
    if(i>0) erase(0, i);
    return *this;
}
SP_HOT string_impl& trim_end(){
    size_type i = size();
    while(i>0 && isspace(c_str()[i-1])) i--;
    if(i<size()) erase(i, size() - i);
    return *this;
}
SP_HOT string_impl& trim(){
    trim_end();
    trim_start();
    return *this;
}
SP_NODISCARD SP_HOT string_impl trimmed() const{
    string_impl temp(*this);
    temp.trim();
    return temp;
}
SP_HOT string_impl& pad_left(size_type total_width, char padding = ' '){
...
SP_HOT string_impl& pad_right(size_type total_width, char padding = ' '){
    size_type len = size();
    bool small = is_small();
    SP_IF_EXPECT(len<total_width){
        if(total_width<=22&&small){
...
SP_NODISCARD SP_PURE size_type count(char c) const{
    const char* dta = data();
    size_type sz = size();
    size_type sum = 0;
...
SP_NODISCARD SP_PURE size_type find(char c, size_type pos = 0) const{
    const char* dta = data();
    size_type sz = size();
    _SP_APPLY_UNROLLED_(sz, SP_IF_NOT_EXPECT(i>=pos&&dta[i]==c) return i);
    return npos;
}
SP_NODISCARD SP_PURE size_type rfind(char c, size_type pos = npos) const{
    const char* dta = data();
    size_type sz = size();
    pos = min(pos, sz-1);
...
SP_NODISCARD SP_PURE size_type find_first_of(const char set, size_type pos = 0) const{
    const char* dta = data();
    size_type sz = size();
    _SP_APPLY_UNROLLED_(sz, SP_IF_NOT_EXPECT(i>=pos&&dta[i]==set) return i);
    return npos;
}
SP_NODISCARD SP_PURE size_type find_last_of(const char set, size_type pos = npos) const{
    const char* dta = data();
    size_type sz = size();
    pos = min(pos, sz-1);
...
SP_NODISCARD SP_PURE size_type find_first_not_of(const char set, size_type pos = 0) const{
    const char* dta = data();
    size_type sz = size();
    _SP_APPLY_UNROLLED_(sz, SP_IF_NOT_EXPECT(i>=pos&&dta[i]!=set) return i);
    return npos;
}
SP_NODISCARD SP_PURE bool starts_with(const char prefix, size_type pos = 0) const{
    const size_type sz = size();
    if(sz) return c_str()[0]==prefix;
    else return false;
}
SP_NODISCARD SP_PURE bool ends_with(const char suffix, size_type pos = npos) const{
    const size_type sz = size();
    if(sz) return c_str()[sz-1]==suffix;
    else return false;
}
SP_NODISCARD SP_PURE bool contains(const char target) const{
    const char* dta = data();
    size_type sz = size();
    _SP_APPLY_UNROLLED_(sz, SP_IF_NOT_EXPECT(dta[i]==target) return true);
    return false;
}
SP_NODISCARD SP_PURE bool starts_with(const char* prefix) const{
    const char* dta = data();
    size_type sz = size();
...
SP_NODISCARD SP_PURE bool ends_with(const char* suffix) const{
    const char* dta = data();
    size_type sz = size();
...
SP_HOT string_impl& replace(size_type pos, size_type len, const char* str){
    return __priv_replace(pos, len, str, strlen(str));
}
SP_HOT string_impl& erase(size_type pos = 0, size_type len = npos){
    size_type sz = size();
...
SP_HOT string_impl& erase_all(char c){
    char* dta = data();
    size_type sz = size();
    size_type new_len = 0;
...
SP_FORCEINLINE void swap(string_impl& other) noexcept{
...
SP_NODISCARD SP_HOT string_impl repeat(size_type times) const{
    string_impl result = *this;
    size_type sz = length();
    for(size_type i = 0; i < times-1; i++) result.__priv_append(c_str(),sz);
    return result;
}
SP_HOT string_impl& replace_all(const char target, const char replacement){
    char* dta = data();
    size_type sz = size();
...
SP_NODISCARD SP_HOT SP_FORCEINLINE string_impl slice(int start, int end) const{ 
...
SP_NODISCARD SP_HOT string_impl substr(size_type pos, size_type len) const { 
...
SP_NODISCARD SP_HOT T split(char delimiter) const{
...
template <typename T, bool remove_empty=false>
SP_NODISCARD SP_HOT T split(const char* delimiter) const{
    const char* ptr = c_str();
    const size_type sz = size();
...
template <typename T>
SP_NODISCARD SP_HOT static string_impl join(const T& parts, const char* sep){
    string_impl result;
...
template <typename T>
SP_NODISCARD SP_HOT string_impl join(const T& parts) const {
    string_impl result;
    if (parts.is_empty()) return result;
    size_type total_len = 0;
    for(size_type i = 0; i < parts.size(); i++) total_len += parts[i].size();
...
SP_HOT string_impl& insert(size_type pos, char c){ return __priv_insert(pos, &c, 1); }
SP_HOT string_impl& insert(size_type pos, size_type count, char c){
    string_impl temp(count, c);
    return __priv_insert(pos, temp.c_str(), count);
}
SP_HOT bool is_numeric() const{
    const char* dta = data();
    size_type sz = size();
...
SP_HOT bool is_whitespace() const{
    const char* dta = data();
    size_type sz = size();
    SP_IF_NOT_EXPECT(sz==0) return false;
    for(size_type i = 0; i < sz; i++){
...
SP_COLD string_impl& wrap(size_type width){
    string_impl result;
    const char* dta = data();
    const size_type sz = size();
    size_type pos = 0;
...
SP_HOT int compare(string_impl& other) const{
    const char* dta = data();
    const size_type sz = size();
...
SP_HOT int compare_case_insensitive(const string_impl& other) const{
    const char* dta = data();
    const size_type sz = size();
    const char* other_dta = other.data();
    const size_type other_sz = other.size();
...
SP_HOT string_impl& fill(char c){
    char* dta = data();
    size_type sz = size();
    memset(dta, c, sz);
    return *this;
}
SP_HOT void resize(size_type n, char c = '\0'){
    size_type sz = size();
    if(n<sz){
        if(is_small()){
            S()._data[n] = '\0';
...
SP_HOT SP_FORCEINLINE string_impl& append(const char* other){ return __priv_append(other, strlen(other)); }
SP_HOT SP_FORCEINLINE string_impl& append(const string_impl& other){ return __priv_append(other.c_str(), other.size()); }
SP_HOT SP_FORCEINLINE string_impl& append(char other){ return __priv_append(&other, 1); }
SP_HOT SP_FORCEINLINE string_impl& append(const char* other, size_type size){ return __priv_append(other, size);}
SP_HOT SP_FORCEINLINE string_impl& append(const string_impl<_other_safety_level,Allocator>& other){ return __priv_append(other.c_str(), other.size()); }

SP_HOT SP_FORCEINLINE string_impl& assign(const char* other){ return __priv_assign(other, strlen(other)); }
SP_HOT SP_FORCEINLINE string_impl& assign(const string_impl& other){ return __priv_assign(other.c_str(), other.size()); }
SP_HOT SP_FORCEINLINE string_impl& assign(char other){ return __priv_assign(&other, 1); }
SP_HOT SP_FORCEINLINE string_impl& assign(const char* other, size_type size){ return __priv_assign(other, size);}
SP_HOT SP_FORCEINLINE string_impl& assign(const string_impl<_other_safety_level,Allocator>& other){ return __priv_assign(other.c_str(), other.size()); }

SP_NODISCARD SP_FORCEINLINE char& at(size_type index){
    _SP_CHECK_SAFETY_LEVEL_(1) SP_IF_NOT_EXPECT(index>=size()) throw std::out_of_range("Index out of range in string_impl at");
    return is_small() ? S()._data[index] : B()._data[index];
}
SP_NODISCARD SP_FORCEINLINE const char& at(size_type index) const{
    _SP_CHECK_SAFETY_LEVEL_(1) SP_IF_NOT_EXPECT(index>=size()) throw std::out_of_range("Index out of range in string_impl at");
    return is_small() ? S()._data[index] : B()._data[index];
}

SP_HOT SP_FORCEINLINE const char& back() const { if(is_big()) return B()._data[B()._size-1]; else return S()._data[(S()._flag & 0x1F) - 1]; }
SP_HOT SP_FORCEINLINE char& back() { if(is_big()) return B()._data[B()._size-1]; else return S()._data[(S()._flag & 0x1F) - 1]; }
SP_HOT SP_FORCEINLINE size_type copy(char* dest, size_type count, size_type pos = 0 ) const{
...
SP_HOT SP_FORCEINLINE const char& front() const{ return c_str()[0]; }
SP_HOT SP_FORCEINLINE char& front() { return data()[0]; }
SP_HOT SP_FORCEINLINE size_type max_size() const { return sp::npos; }

SP_HOT SP_FORCEINLINE string_impl& push_back(char element) { return __priv_push_back(element); }
SP_HOT SP_FORCEINLINE string_impl& refresh_size(){
...
SP_HOT SP_FORCEINLINE string_impl<level> operator*(size_type multiplier, const string_impl<level>& str){
    return str * multiplier;
}

template <short level, template <typename> class alloc, bool use_sso>
SP_FORCEINLINE bool operator==(const char* other,const string_impl<level, alloc, use_sso>& str){
    return str==other;
}
