...
#else
    template<typename T>
    struct is_unsigned : bool_constant<is_arithmetic_v<T> && !is_signed<T>::value> {};
#endif
...
// --- Destructibility family ---
#if __has_builtin(__is_destructible)
    template<typename T> struct is_destructible : bool_constant<__is_destructible(T)> {};
#else
    // Most complete types are destructible; this is a reasonable fallback
    template<typename T> struct is_destructible : true_type {};
#endif
...
#else
    template<typename T> struct is_nothrow_destructible : is_destructible<T> {}; // conservative
#endif
